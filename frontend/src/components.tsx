  ...
  return (
    <article className="movie-card">
      <Link to={`/movies/${movie.id}`}>
        ...
        <h3>{movie.title}</h3>
        <p>{movie.releaseDate?.slice(0, 4) ?? 'Release date unknown'}</p>
      </Link>
      {onToggle && (
        <button
          ...
          aria-label={
            wishlisted ? `Remove ${movie.title} from wishlist` : `Add ${movie.title} to wishlist`
          }
        >
    ...
    <div className="movie-grid">
      {movies.map((movie) => (
        <MovieCard
          key={movie.id}
          movie={movie}
          wishlisted={wishlist.some((item) => item.id === movie.id)}
          onToggle={onToggle}
        />
      ))}
    </div>
