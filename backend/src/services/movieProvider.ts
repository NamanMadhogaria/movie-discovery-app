...
type TmdbMovie = { id: number; title?: string; name?: string; overview?: string; poster_path?: string | null; backdrop_path?: string | null; release_date?: string; first_air_date?: string; vote_average?: number; vote_count?: number; genre_ids?: number[]; genres?: { name: string }[]; runtime?: number | null };
type TmdbPage = { page: number; total_pages: number; total_results: number; results: TmdbMovie[] };
const image = (path?: string | null, size = 'w500') => path ? `https://image.tmdb.org/t/p/${size}${path}` : null;

const genreNames: Record<number, string> = { 28: 'Action', 12: 'Adventure', 16: 'Animation', 35: 'Comedy', 80: 'Crime', 99: 'Documentary', 18: 'Drama', 10751: 'Family', 14: 'Fantasy', 27: 'Horror', 9648: 'Mystery', 10749: 'Romance', 878: 'Science Fiction', 53: 'Thriller', 10752: 'War', 37: 'Western' };

export function mapMovie(item: TmdbMovie): Movie {
  return { id: item.id, title: item.title ?? item.name ?? 'Untitled', overview: item.overview?.trim() || 'No description available.', posterUrl: image(item.poster_path), backdropUrl: image(item.backdrop_path, 'original'), releaseDate: item.release_date ?? item.first_air_date ?? null, rating: typeof item.vote_average === 'number' ? Math.round(item.vote_average * 10) / 10 : null, voteCount: item.vote_count ?? 0, genres: item.genres?.map(g => g.name) ?? item.genre_ids?.map(id => genreNames[id]).filter(Boolean) ?? [], runtimeMinutes: item.runtime ?? null };
}
...
const demoPage = (items: Movie[], page: number): PageResult<Movie> => ({ data: items, pagination: { page, totalPages: 1, totalResults: items.length, hasNextPage: false } });
